...
if [ "${1}" = "-R" ]; then
  sed -i 's/^Icon=.*/Icon=firefox-developer-edition/' /usr/share/applications/firefoxdeveloperedition.desktop
  sed -i 's/^Exec=.*/Exec=\/opt\/apps\/com.qq.office.deepin\/files\/run.sh/' /usr/share/applications/com.qq.office.deepin.desktop
else
  sed -i 's/^Icon=.*/Icon=firefox/' /usr/share/applications/firefoxdeveloperedition.desktop
